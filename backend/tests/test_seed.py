...
async def test_lessons_per_module_align_with_spec(test_db_session: AsyncSession):
    ...
    for module_lessons_data in LESSONS_DATA:
        ...
        result = await test_db_session.execute(select(Lesson).where(Lesson.module_id == module.id))
        lessons = result.scalars().all()

        assert len(lessons) == expected_lesson_count, (
            f"Module {module_order} should have {expected_lesson_count} lessons"
        )
...
async def test_lessons_have_russian_content(test_db_session: AsyncSession):
    ...
    for lesson in lessons:
        ...
        assert content_has_russian or theory_has_russian, (
            f"Lesson {lesson.title} should have Russian content"
        )
...
async def test_module_order_is_sequential(test_db_session: AsyncSession):
    ...
    for i, module in enumerate(modules, start=1):
        assert module.order == i, (
            f"Module order should be sequential, expected {i}, got {module.order}"
        )
